...
public class CookieLocaleContextResolver extends AcceptHeaderLocaleContextResolver {
	...
	public static final String DEFAULT_NAME = CookieLocaleContextResolver.class.getName() + ".LOCALE";
	...
	private void parseCookieIfNecessary(ServerWebExchange exchange) {
		if (exchange.getAttribute(DEFAULT_NAME) == null) {
			...
			if (cookie != null) {
				...
				context = new SimpleLocaleContext(locale);
			}
			exchange.getAttributes().put(DEFAULT_NAME, context != null ? context : super.resolveLocaleContext(exchange));
		}
	}
	...
	public void setLocaleContext(ServerWebExchange exchange, LocaleContext lc) {
		...
		if (lc != null && lc.getLocale() != null) {
			...
			var cookie = ResponseCookie.from(DEFAULT_NAME, lc.getLocale().toLanguageTag()).build();
